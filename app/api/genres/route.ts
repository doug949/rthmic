...
export async function POST(req: NextRequest) {
  ...
  try {
    await client.set(
      `genres:${uid}`,
      JSON.stringify(genres.map((g: string) => titleCaseStyle(g))),
      { EX: 60 * 60 * 24 * 365 }
    );
