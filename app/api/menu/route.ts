...
export async function POST(request: NextRequest) {
  ...
  const { slug, songs } = body as { slug: string; songs: SavedRhythm[] };
  if (!slug || !Array.isArray(songs)) {
    return NextResponse.json({ error: "slug and songs required" }, { status: 400 });
  }

  try {
    await withRedis(async (client) => {
      await client.set(menuKey(uid, slug), JSON.stringify(songs));
    });
