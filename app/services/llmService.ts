...
function buildUserPrompt(pillar: PillarType, transcript: string): string {
  ...
  if (/Developer experiment:\s*Photograph to Rthm/i.test(transcript)) {
    return `${base}\n\nCRITICAL PHOTO RTHM RULES:\n1. Keep the title, state summary, and intent anchored to the exact photographed subject.\n2. Preserve supplied proper nouns, addresses, map labels, reverse geocode labels, OpenStreetMap places, Wikipedia places, coordinates, EXIF, distances, transport, amenities, and local facts. Do not reduce them to generic phrases like "this building" or "the area".\n3. Give findings, not assignments. Identify visible architecture, materials, layout, condition, light, aspect, setting, and other relevant features, then explain what those features mean.\n4. ZERO HOMEWORK: never tell the listener to look, notice, inspect, check, verify, research, investigate, ask, find out, consider, explore, or remember to do something. Never turn missing information into a question.\n5. For property use, directly state supplied nearby transport, amenities, landmarks, distances, access, aspect/light, and visible property features. Connect them to practical implications without creating a viewing checklist.\n6. Never invent facts. If a detail is neither visible nor supplied, omit it entirely.\n7. The title should name the exact subject/place when a name or address is supplied.`;
  }

  if (/Developer experiment:\s*Walking Tour/i.test(transcript)) {
    return `${base}\n\nCRITICAL EXPERIMENT RULES:\n1. Keep the title, state summary, and intent anchored to the exact photographed subject or exact walking location described.\n2. If the transcript contains proper nouns, street names, building names, map labels, reverse geocode labels, named OpenStreetMap places, named Wikipedia places, coordinates, EXIF, or local facts, preserve the strongest names and facts in the brief. Do not reduce them to generic phrases like "this building" or "the area".\n3. Do not generalise a specific wall, building, property, street, route, or place into a generic song about that type of thing.\n4. Do not invent history, landmarks, ownership, surroundings, construction dates, pub names, or business/finance context. If details are uncertain, omit them or frame them as precise questions to verify.\n5. The title should name the exact subject/place when a name or address is supplied.`;
  }
...
Use the ${pillar} creative structure silently. Never name, explain, question, or discuss that structure in the lyrics. Write the complete RTHMIC song now.`;
  ...
  if (/Developer experiment:\s*Photograph to Rthm/i.test(transcript)) {
    return `${base}\n\nCRITICAL PHOTO RTHM RULES:\n1. EXACT SUBJECT FIRST - write about the photographed place or object, not a generic version of it.\n2. NAME AND FACT TEST - use supplied proper nouns, addresses, map labels, reverse geocode labels, OpenStreetMap places, Wikipedia places, coordinates, EXIF details, distances, named transport, amenities, and local facts in the title and lyrics.\n3. EXPLAIN THE EVIDENCE - state visible architectural, material, layout, condition, light, aspect, setting, and contextual features and explain their significance. Do not instruct the listener to look for them.\n4. ZERO HOMEWORK - the lyrics must never ask or tell the listener to look, notice, inspect, check, verify, research, investigate, ask, find out, consider, explore, or remember to do anything. Do not include questions, a viewing checklist, calls to action, or unresolved research prompts.\n5. PROPERTY ANSWERS - when the purpose concerns buying or viewing a property, directly state supplied facts about nearby transport, amenities, landmarks, distances, access, aspect/light, setting, and visible building features, then explain their grounded practical implications.\n6. NO VAGUE CONDITIONALS - use actual supplied names and facts. Do not write hypothetical lines such as "if there is a station nearby".\n7. TRUTH BOUNDARIES - never invent history, dates, ownership, condition beyond visible evidence, transport, amenities, listing details, or surroundings. Missing information must be omitted, not handed back as a task.\n8. NO CATEGORY DRIFT - do not turn property, walls, buildings, or streets into generic finance/business songs unless money or business is explicitly the point.\n9. OPENING STANDARD - the first four lyric lines must immediately establish the exact photographed subject using the strongest available name, address, visible feature, and local fact.`;
  }

  if (/Developer experiment:\s*Walking Tour/i.test(transcript)) {
    return `${base}\n\nCRITICAL EXPERIMENT RULES:\n1. SPECIFIC SUBJECT FIRST - write about the exact photographed place/object or exact walking location described in the transcript, not a generic version of it.\n2. NAME TEST - if the transcript contains a building name, street, park, district, map label, reverse geocode label, named OpenStreetMap place, named Wikipedia place, coordinate, or user-supplied address, the title and first verse must use the strongest available proper noun(s). A lyric that only says "this building", "the balconies", "the street", or "the area" when names are available is a failure.\n3. FACTS BEFORE QUESTIONS - if the transcript contains 'SOURCE FACTS TO USE', 'Local facts from geodata', named Wikipedia places, OpenStreetMap features, reverse geocode labels, dates, distances, map labels, coordinates, EXIF metadata, or source-backed nearby facts, weave those names and facts directly into the lyric before offering questions.\n4. NO VAGUE CONDITIONALS - avoid lines like 'if there is a pub on the corner' or 'there may have been...' when supplied facts name the actual pub, landmark, street, building, site, or nearby place. Use the actual names and supplied dates/details. Only use uncertainty when the supplied facts are genuinely incomplete.\n5. TRUTH BOUNDARIES - never invent history, construction years, owners, families, pub names, landmarks, or famous nearby places that are not in the transcript/local facts/image brief/user context. Known facts can be stated; missing facts should simply be omitted or framed as a precise thing to verify.\n6. LOCATION ANCHOR - when coordinates, map labels, capture time, user context, or specific visual details are present, weave those details naturally into the lyric.\n7. NO CATEGORY DRIFT - do not turn property, walls, buildings, streets, or walks into generic finance/business songs unless money or business is explicitly the point.\n8. USEFULNESS - make the song help the listener see this place more clearly: named things nearby, what to notice, what the known history changes, what questions still matter, and what clues are visible in front of them.\n9. OPENING STANDARD - the first four lyric lines should immediately convince the listener this was made for this exact place. Prefer "The Pines on Herbert Park Lane..." over "You're standing in front of a building..." when that name is available.`;
  }
...
export async function interpretBrief(rawTranscript: string, overridePillar?: PillarType): Promise<LLMBriefResult> {
  ...
  const message = await client.messages.create({
    ...
    messages: [
      {
        role: "user",
        content: `User's spoken state:\n"${transcript}"\n\nThe pillar is already determined: ${pillar}. Return only the brief JSON. Do not write lyrics.${/Developer experiment:\s*(Photograph to Rthm|Walking Tour)/i.test(transcript) ? "\n\nFor this developer experiment, keep the title and brief anchored to the exact photographed subject or walking location. Prefer named local facts from geodata/Wikipedia/OpenStreetMap over generic possibilities. Do not invent history, landmarks, ownership, surroundings, business, or finance context. Preserve coordinates, EXIF, map label, user context, named nearby places, distances, dates, and visible clues when present. If a proper name/address/place is supplied, use it in the title and state summary; do not collapse it into generic words like 'building', 'street', or 'area'." : ""}${/Developer experiment:\s*Photograph to Rthm/i.test(transcript) ? "\n\nPHOTO-SPECIFIC REQUIREMENT: produce direct findings and explanations, never questions or homework. Do not tell the listener to look, notice, inspect, check, verify, research, investigate, ask, find out, consider, or explore. For property photos, directly state supplied transport, amenities, distances, access, aspect/light, setting, and visible building features. Omit unavailable facts instead of assigning them to the listener." : ""}`,
      },
    ],
  });
