...
The pillar is already determined: ${pillar}. Write the complete RTHMIC lyrics now.`;
  ...
  if (/Developer experiment:\s*(Photograph to Rthm|Walking Tour)/i.test(transcript)) {
    return `${base}\n\nCRITICAL EXPERIMENT RULES:\n1. SPECIFIC SUBJECT FIRST - write about the exact photographed place/object or the exact walking location described in the transcript, not a generic version of it.\n2. FACTS BEFORE QUESTIONS - if the transcript contains 'Local facts from geodata', named Wikipedia places, OpenStreetMap features, reverse geocode labels, dates, distances, map labels, coordinates, EXIF metadata, or source-backed nearby facts, weave those names and facts directly into the lyric.\n3. NO VAGUE CONDITIONALS - avoid lines like 'if there is a pub on the corner' or 'there may have been...' when supplied facts name the actual pub, landmark, street, building, site, or nearby place. Use the actual names and supplied dates/details. Only use uncertainty when the supplied facts are genuinely incomplete.\n4. TRUTH BOUNDARIES - never invent history, construction years, owners, families, pub names, landmarks, or famous nearby places that are not in the transcript/local facts/image brief/user context. Known facts can be stated; missing facts should simply be omitted or framed as a precise thing to verify.\n5. LOCATION ANCHOR - when coordinates, map labels, capture time, user context, or specific visual details are present, weave those details naturally into the lyric.\n6. NO CATEGORY DRIFT - do not turn property, walls, buildings, streets, or walks into generic finance/business songs unless money or business is explicitly the point.\n7. USEFULNESS - make the song help the listener see this place more clearly: named things nearby, what to notice, what the known history changes, what questions still matter, and what clues are visible in front of them.`;
  }
